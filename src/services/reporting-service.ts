  ...
  generateStakeholderReport(stakeholderId: string): string {
    ...
    if (issuances.length > 0) {
      ...
      for (const issuance of issuances) {
        ...
        lines.push(
          `  - ${issuance.qty.toLocaleString()} shares of ${sc?.label ?? issuance.securityClassId}`
        );
    ...
    if (grants.length > 0) {
      ...
      for (const grant of grants) {
        ...
        if (grant.vesting) {
          lines.push(
            `    Vesting: ${grant.vesting.monthsTotal} months, ${grant.vesting.cliffMonths} month cliff`
          );
    ...
    if (sc.kind === 'OPTION_POOL') {
      ...
      if (issuances.length > 0) {
        ...
        for (const issuance of issuances) {
          const stakeholder = this.stakeholderService.getStakeholder(issuance.stakeholderId);
          lines.push(
            `  - ${stakeholder?.name ?? issuance.stakeholderId}: ${issuance.qty.toLocaleString()} shares`
          );
        }
      }
    }
